    ...
    return (
        <>
            <nav className={`${toggle ? 'w-64' : 'w-20'} z-50 fixed top-0 s-0 h-screen flex`}>
                <div className={`${toggle ? 'sidebar w-44 ps-5 h-full bg-black text-white flex flex-col justify-between transition-all duration-300' : '-ms-44 w-44 ps-5 h-full bg-black text-white flex flex-col justify-between transition-all duration-300'}`} style={{ willChange: 'transform' }}>
                    <div className="sidebar-links">
                        <ul>
                            <li onClick={() => setToggle(false)} className='pt-6 hover:text-red-700 transition-all duration-300'>
